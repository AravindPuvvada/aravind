...
vibration_pin = 16  # GPIO pin number where the vibration sensor is connected
...
while True:
    ...
    if GPIO.input(vibration_pin) == GPIO.LOW:  # If vibration is detected (HIGH)
        print("Vibration detected!!")
    else:
        print("No vibration")  # No vibration (LOW)
        ...
        if pir_state == 0:
            if name == "Unknown":
                ...
                if GPIO.input(vibration_pin) == GPIO.LOW:  # If vibration is detected (HIGH)
                    ...
                    body="Alert!! Unknown person vibrations are detected .",
                  )
                buzzer_alert()
